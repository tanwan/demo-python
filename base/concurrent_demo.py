...
class ConcurrentDemo(unittest.TestCase):
    ...
    def test_executor_map(self):
        """executro map方法"""
        # 因为executro_map需要2个参数,因此这边就需要2个列表,前一个表示调用executro_map的第一个参数,后面的列表表示第二个参数
        results1 = executor.map(executro_map, ["use variable", "use variable", "use variable", "use variable"], [1, 2, 3, 4])
        ...
        # 使用列表进行拆包
        results2 = executor.map(executro_map, *lst)
